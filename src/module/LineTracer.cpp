...
LineTracer::LineTracer(int targetBrightness_, bool isLeftCourse_)
  : targetBrightness(targetBrightness_), isLeftCourse(isLeftCourse_)
{
}
...
void LineTracer::run(NormalCourseProperty& settings)
{
  ...
  while(currentDistance - initialDistance < settings.targetDistance) {
    ...
    // 前進値の計算
    speedValue = 30;
    speedValue = speedControl.calculateSpeed(settings.speedPid.kp, settings.speedPid.ki,
                                             settings.speedPid.kd, settings.targetSpeed);
